...
// Supports edit mode, deletion and favorite toggle
...
function GameCard({
  ...
  // Enable edit mode and initialize form state
  const startEdit = () => {
    setEditTitle(title);
  ...
  // Validate and save edited game data
  const saveEdit = () => {
    if (!editTitle.trim()) {
    ...
    const updated = {
      ...
      status: editStatus,
      hoursPlayed: Number(editHours) || 0,
      ...
      {!editMode ? (
        <div>
          <div className="flex items-start justify-between mb-3">
            <h2 className="font-semibold text-lg flex items-center gap-2">
              <button
                ...
                className="text-lg leading-none opacity-90 hover:scale-110 transition-transform"
            ...
            <span className="text-xs px-2 py-1 rounded-full bg-slate-700 uppercase tracking-wide">
              {platform}
            </span>
          </div>
          ...
          <div className="flex justify-end pt-5">
            <button
              onClick={startEdit}
              className="text-xs text-slate-300 hover:text-white pe-4 py-1 rounded"
              title="Modifier"
            >
              Modifier
            </button>
            <button
              onClick={onDelete}
              className="text-xs text-red-300 hover:text-red-400 py-1 rounded"
              title="Supprimer"
            >
              Supprimer
            </button>
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-end gap-2 pt-5">
            <button
              onClick={cancelEdit}
              className="text-sm px-3 py-1 rounded border border-slate-700 text-slate-300"
            >
              Annuler
            </button>
            <button
